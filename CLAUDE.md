...
- `lib/personas/` — **everything about the 16 HMTI personas/avatars in one folder** (import from `@/lib/personas`). The catalogue (`data.ts`: `PERSONAS` + `HMTI_AXES`) is the single source of truth for copy, descriptions, and avatar visuals. `compatibility.ts`/`persona.ts` hold the **deterministic hash-based placeholders** `computeCompatibility()` (score + categories) and `computePersona()` (the 16-type HMTI built from 4 binary axes). Same inputs → same output. `CompatibilityResult` and `Persona` (in `types.ts`) are the shared contracts imported by the flow routes, `ResultsView`, and `PersonaCard` — change the shape here, not in copies. See `lib/personas/README.md`.

### The open seam (most likely next work)
System 1 produces real transcripts + personas; system 2's result still comes from `computeCompatibility(initiatorInput, roommateInput)` over placeholder strings. Connecting them — feeding interview transcripts/personas into the flow and replacing the hash stubs — is the central unfinished integration. Stubs are marked `TODO`: `JoinForm` and `StartMatchingForm` (raw `Textarea`s instead of voice interview), `lib/personas/` (hash placeholders in `compatibility.ts`/`persona.ts`), and both stores.
